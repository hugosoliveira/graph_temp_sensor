...
def cooling(pairs_indexes_cooling, df, number_cycles,t_min,t_max,t_room):
    ...
    for cycle in range(1, number_cycles+1):
        ...
        quantity_numbers = cooling_values['T_'+ str(cycle)].count()
        #Linear temperature Changes -
        # temperatures = pd.DataFrame(np.linspace(80, 25, quantity_numbers).tolist(), columns=['Temp_'+str(cycle)])
    ...
    # print('cooling')
    # print(cooling_values)
    return final_cooling_data, cooling_values
